...
public class OpenIddictDataSeedContributor : OpenIddictDataSeedContributorBase, IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // AdminConsole Client
        var adminConsoleClientId = configurationSection["AcroStack_AdminConsole:ClientId"];
        if (!adminConsoleClientId.IsNullOrWhiteSpace())
        {
            var adminConsoleRootUrl = configurationSection["AcroStack_AdminConsole:RootUrl"]?.TrimEnd('/');
            await CreateOrUpdateApplicationAsync(
                applicationType: OpenIddictConstants.ApplicationTypes.Web,
                name: adminConsoleClientId,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Admin Console Application",
                secret: null,
                grantTypes: new List<string>
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode,
                ...
                scopes: commonScopes,
                redirectUris: new List<string> { $"{adminConsoleRootUrl}/", $"{adminConsoleRootUrl}/silent-renew.html" },
                postLogoutRedirectUris: new List<string> { $"{adminConsoleRootUrl}/" },
                clientUri: adminConsoleRootUrl,
