...
import type {
  ...
  FieldValue,
  ObjectFormValues,
  TypedDatePickerFieldComponent,
} from "../types";
import { normalizeFieldErrors, shouldShowFieldError } from "../utils/errors";
import { resolveFieldMode } from "../utils/mode";
import { buildValidationProps, getDateViewValue } from "../utils/props";

export function createDatePickerFieldComponent<TFormValues extends ObjectFormValues>(
  form: AntdFormApi<TFormValues>,
): TypedDatePickerFieldComponent<TFormValues> {
  type DatePickerValue = ComponentProps<typeof DatePicker>["value"];

  return function DatePickerField<TName extends DatePickerFieldName<TFormValues>>(
            ...
            {(isSubmitted) => {
              ...
              if (resolved.mode === "view") {
                return (
                  <AntdForm.Item
                    ...
                    tooltip={tooltip}
                  >
                    <span>{getDateViewValue(field.state.value)}</span>
                  </AntdForm.Item>
                );
              }
              ...
              return (
                ...
                >
                  <DatePicker
                    {...datePickerProps}
                    value={(field.state.value ?? null) as DatePickerValue}
