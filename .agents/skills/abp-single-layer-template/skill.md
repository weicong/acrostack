...
│       ├── react/             # React SPA (separate folder)
│       └── AcroStackModule.cs
```
...
Configure entities in `AcroStackDbContext`:
...
This solution uses **React** for the UI layer.

- React app is in the `react/` folder
- Uses Vite as the build tool and dev server
- Run `abp generate-proxy -t js` to update API proxies
