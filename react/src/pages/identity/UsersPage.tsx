...
function SortIcon({ field, sort }: { field: string; sort: SortState | null }) {
  if (!sort || sort.field !== field) return <ChevronsUpDown className="ml-1 inline h-3.5 w-3.5 opacity-50" />
...
export function UsersPage() {
  ...
  const adminConsoleUrl = `${getAdminConsoleUrl().replace(/\/$/, '')}/identity/users`
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            <div className="ml-auto">
              <Button
                variant="outline"
                size="sm"
                onClick={() => window.location.assign(adminConsoleUrl)}
              >
