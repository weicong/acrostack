...
export const appUserGetListQueryKey = ({ query }: Omit<AppUserGetListOptions, "headers"> = {}) =>
  [{ url: "/api/app/app-user" }, ...(query ? [query] : [])] as const;

type AppUserGetListQueryKey = ReturnType<typeof appUserGetListQueryKey>;

export function appUserGetListQueryOptions(
  { query }: AppUserGetListOptions = {},
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await appUserGetList({
        ...config,
        query,
...
>(
  {
    query,
  }: { query?: AppUserGetListOptions["query"] | (() => AppUserGetListOptions["query"]) } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
        AppUserGetListStatus200,
