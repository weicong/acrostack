...
} from "../../models/user/UserGetRoles";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { userGetRoles } from "../../clients/user/userGetRoles";

export const userGetRolesQueryKey = ({ path }: Omit<UserGetRolesOptions, "headers">) =>
  [{ url: "/api/identity/users/:id/roles", params: path }] as const;

type UserGetRolesQueryKey = ReturnType<typeof userGetRolesQueryKey>;

export function userGetRolesQueryOptions(
  { path }: UserGetRolesOptions,
...
>(
  { path }: { path: UserGetRolesOptions["path"] | (() => UserGetRolesOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        UserGetRolesStatus200,
