...
export const userFindByEmailQueryKey = ({ path }: Omit<UserFindByEmailOptions, "headers">) =>
  [{ url: "/api/identity/users/by-email/:email", params: path }] as const;

type UserFindByEmailQueryKey = ReturnType<typeof userFindByEmailQueryKey>;

export function userFindByEmailQueryOptions(
  { path }: UserFindByEmailOptions,
...
>(
  { path }: { path: UserFindByEmailOptions["path"] | (() => UserFindByEmailOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        UserFindByEmailStatus200,
