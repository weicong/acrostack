...
export const menuItemAdminGetPageLookupQueryKey = ({
  query,
}: Omit<MenuItemAdminGetPageLookupOptions, "headers"> = {}) =>
  [{ url: "/api/cms-kit-admin/menu-items/lookup/pages" }, ...(query ? [query] : [])] as const;
...
export function menuItemAdminGetPageLookupQueryOptions(
  { query }: MenuItemAdminGetPageLookupOptions = {},
...
>(
  {
    ...
    query?:
      | MenuItemAdminGetPageLookupOptions["query"]
      | (() => MenuItemAdminGetPageLookupOptions["query"]);
  } = {},
  options: {
    query?: Partial<
