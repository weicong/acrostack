...
export const blogPostAdminGetQueryKey = ({ path }: Omit<BlogPostAdminGetOptions, "headers">) =>
  [{ url: "/api/cms-kit-admin/blogs/blog-posts/:id", params: path }] as const;

type BlogPostAdminGetQueryKey = ReturnType<typeof blogPostAdminGetQueryKey>;

export function blogPostAdminGetQueryOptions(
  { path }: BlogPostAdminGetOptions,
...
>(
  { path }: { path: BlogPostAdminGetOptions["path"] | (() => BlogPostAdminGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        BlogPostAdminGetStatus200,
