...
export const auditLogGetEntityChangesQueryKey = ({
  path,
}: Omit<AuditLogGetEntityChangesOptions, "headers">) =>
  [{ url: "/api/app/audit-log/entity-changes/:auditLogId", params: path }] as const;
...
export function auditLogGetEntityChangesQueryOptions(
  { path }: AuditLogGetEntityChangesOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await auditLogGetEntityChanges({
        ...config,
        path,
...
>(
  {
    ...
    path: AuditLogGetEntityChangesOptions["path"] | (() => AuditLogGetEntityChangesOptions["path"]);
  },
  options: {
    query?: Partial<
      QueryObserverOptions<
