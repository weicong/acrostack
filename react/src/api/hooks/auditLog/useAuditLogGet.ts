...
} from "../../models/auditLog/AuditLogGet";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { auditLogGet } from "../../clients/auditLog/auditLogGet";

export const auditLogGetQueryKey = ({ path }: Omit<AuditLogGetOptions, "headers">) =>
  [{ url: "/api/app/audit-log/:id", params: path }] as const;

type AuditLogGetQueryKey = ReturnType<typeof auditLogGetQueryKey>;

export function auditLogGetQueryOptions(
  { path }: AuditLogGetOptions,
...
>(
  { path }: { path: AuditLogGetOptions["path"] | (() => AuditLogGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        AuditLogGetStatus200,
