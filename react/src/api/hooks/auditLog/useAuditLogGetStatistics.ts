...
export const auditLogGetStatisticsQueryKey = ({
  query,
}: Omit<AuditLogGetStatisticsOptions, "headers"> = {}) =>
  [{ url: "/api/app/audit-log/statistics" }, ...(query ? [query] : [])] as const;
...
export function auditLogGetStatisticsQueryOptions(
  { query }: AuditLogGetStatisticsOptions = {},
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await auditLogGetStatistics({
        ...config,
        query,
...
>(
  {
    ...
    query?: AuditLogGetStatisticsOptions["query"] | (() => AuditLogGetStatisticsOptions["query"]);
  } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
