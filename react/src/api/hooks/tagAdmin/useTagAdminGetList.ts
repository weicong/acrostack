...
export const tagAdminGetListQueryKey = ({ query }: Omit<TagAdminGetListOptions, "headers"> = {}) =>
  [{ url: "/api/cms-kit-admin/tags" }, ...(query ? [query] : [])] as const;

type TagAdminGetListQueryKey = ReturnType<typeof tagAdminGetListQueryKey>;

export function tagAdminGetListQueryOptions(
  { query }: TagAdminGetListOptions = {},
...
>(
  {
    query,
  }: { query?: TagAdminGetListOptions["query"] | (() => TagAdminGetListOptions["query"]) } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
        TagAdminGetListStatus200,
