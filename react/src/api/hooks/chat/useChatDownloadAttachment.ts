...
export const chatDownloadAttachmentQueryKey = ({
  path,
}: Omit<ChatDownloadAttachmentOptions, "headers">) =>
  [{ url: "/api/app/chat/messages/:messageId/attachment", params: path }] as const;
...
export function chatDownloadAttachmentQueryOptions(
  { path }: ChatDownloadAttachmentOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await chatDownloadAttachment({
        ...config,
        path,
...
>(
  {
    ...
    path: ChatDownloadAttachmentOptions["path"] | (() => ChatDownloadAttachmentOptions["path"]);
  },
  options: {
    query?: Partial<
      QueryObserverOptions<
