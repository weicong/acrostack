...
} from "../../models/role/RoleGetList";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { roleGetList } from "../../clients/role/roleGetList";

export const roleGetListQueryKey = ({ query }: Omit<RoleGetListOptions, "headers"> = {}) =>
  [{ url: "/api/identity/roles" }, ...(query ? [query] : [])] as const;

type RoleGetListQueryKey = ReturnType<typeof roleGetListQueryKey>;

export function roleGetListQueryOptions(
  { query }: RoleGetListOptions = {},
...
>(
  { query }: { query?: RoleGetListOptions["query"] | (() => RoleGetListOptions["query"]) } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
        RoleGetListStatus200,
