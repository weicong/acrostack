...
export const identityClaimTypeGetQueryKey = ({
  path,
}: Omit<IdentityClaimTypeGetOptions, "headers">) =>
  [{ url: "/api/app/identity-claim-type/:id", params: path }] as const;
...
export function identityClaimTypeGetQueryOptions(
  { path }: IdentityClaimTypeGetOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await identityClaimTypeGet({
        ...config,
        path,
...
>(
  {
    path,
  }: { path: IdentityClaimTypeGetOptions["path"] | (() => IdentityClaimTypeGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        IdentityClaimTypeGetStatus200,
