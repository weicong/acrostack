...
export const blogPostPublicGetQueryKey = ({ path }: Omit<BlogPostPublicGetOptions, "headers">) =>
  [{ url: "/api/cms-kit-public/blog-posts/:blogSlug/:blogPostSlug", params: path }] as const;

type BlogPostPublicGetQueryKey = ReturnType<typeof blogPostPublicGetQueryKey>;

export function blogPostPublicGetQueryOptions(
  { path }: BlogPostPublicGetOptions,
...
>(
  { path }: { path: BlogPostPublicGetOptions["path"] | (() => BlogPostPublicGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        BlogPostPublicGetStatus200,
