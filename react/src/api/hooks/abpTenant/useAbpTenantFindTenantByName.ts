...
export const abpTenantFindTenantByNameQueryKey = ({
  path,
}: Omit<AbpTenantFindTenantByNameOptions, "headers">) =>
  [{ url: "/api/abp/multi-tenancy/tenants/by-name/:name", params: path }] as const;
...
export function abpTenantFindTenantByNameQueryOptions(
  { path }: AbpTenantFindTenantByNameOptions,
...
>(
  {
    ...
    path:
      | AbpTenantFindTenantByNameOptions["path"]
      | (() => AbpTenantFindTenantByNameOptions["path"]);
  },
  options: {
    query?: Partial<
