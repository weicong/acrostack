...
export const abpTenantFindTenantByIdQueryKey = ({
  path,
}: Omit<AbpTenantFindTenantByIdOptions, "headers">) =>
  [{ url: "/api/abp/multi-tenancy/tenants/by-id/:id", params: path }] as const;
...
export function abpTenantFindTenantByIdQueryOptions(
  { path }: AbpTenantFindTenantByIdOptions,
...
>(
  {
    ...
    path: AbpTenantFindTenantByIdOptions["path"] | (() => AbpTenantFindTenantByIdOptions["path"]);
  },
  options: {
    query?: Partial<
      QueryObserverOptions<
