...
export const backgroundJobGetListQueryKey = ({
  query,
}: Omit<BackgroundJobGetListOptions, "headers"> = {}) =>
  [{ url: "/api/app/background-job" }, ...(query ? [query] : [])] as const;
...
export function backgroundJobGetListQueryOptions(
  { query }: BackgroundJobGetListOptions = {},
...
>(
  {
    ...
    query?: BackgroundJobGetListOptions["query"] | (() => BackgroundJobGetListOptions["query"]);
  } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
