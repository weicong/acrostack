...
import type {
  ...
  BookGetStatus500,
  BookGetStatus501,
} from "../../models/book/BookGet";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { bookGet } from "../../clients/book/bookGet";

export const bookGetQueryKey = ({ path }: Omit<BookGetOptions, "headers">) =>
  [{ url: "/api/app/book/:id", params: path }] as const;
...
>(
  { path }: { path: BookGetOptions["path"] | (() => BookGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        BookGetStatus200,
