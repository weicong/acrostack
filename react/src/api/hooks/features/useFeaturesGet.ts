...
} from "../../models/features/FeaturesGet";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { featuresGet } from "../../clients/features/featuresGet";

export const featuresGetQueryKey = ({ query }: Omit<FeaturesGetOptions, "headers"> = {}) =>
  [{ url: "/api/feature-management/features" }, ...(query ? [query] : [])] as const;

type FeaturesGetQueryKey = ReturnType<typeof featuresGetQueryKey>;

export function featuresGetQueryOptions(
  { query }: FeaturesGetOptions = {},
...
>(
  { query }: { query?: FeaturesGetOptions["query"] | (() => FeaturesGetOptions["query"]) } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
        FeaturesGetStatus200,
