...
export const permissionsGetQueryKey = ({ query }: Omit<PermissionsGetOptions, "headers"> = {}) =>
  [{ url: "/api/permission-management/permissions" }, ...(query ? [query] : [])] as const;

type PermissionsGetQueryKey = ReturnType<typeof permissionsGetQueryKey>;

export function permissionsGetQueryOptions(
  { query }: PermissionsGetOptions = {},
...
>(
  {
    query,
  }: { query?: PermissionsGetOptions["query"] | (() => PermissionsGetOptions["query"]) } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
        PermissionsGetStatus200,
