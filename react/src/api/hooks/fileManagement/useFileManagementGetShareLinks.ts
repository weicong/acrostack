...
import type {
  FileManagementGetShareLinksOptions,
  FileManagementGetShareLinksStatus200,
} from "../../models/fileManagement/FileManagementGetShareLinks";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { fileManagementGetShareLinks } from "../../clients/fileManagement/fileManagementGetShareLinks";

export const fileManagementGetShareLinksQueryKey = ({
  path,
}: Omit<FileManagementGetShareLinksOptions, "headers">) =>
  [{ url: "/api/app/file-management/files/:id/share-links", params: path }] as const;
...
export function fileManagementGetShareLinksQueryOptions(
  { path }: FileManagementGetShareLinksOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await fileManagementGetShareLinks({
        ...config,
        path,
...
>(
  {
    ...
    path:
      | FileManagementGetShareLinksOptions["path"]
      | (() => FileManagementGetShareLinksOptions["path"]);
  },
  options: {
    query?: Partial<
