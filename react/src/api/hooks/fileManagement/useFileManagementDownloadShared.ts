...
export const fileManagementDownloadSharedQueryKey = ({
  path,
}: Omit<FileManagementDownloadSharedOptions, "headers">) =>
  [{ url: "/api/app/file-management/shared/:token", params: path }] as const;
...
export function fileManagementDownloadSharedQueryOptions(
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await fileManagementDownloadShared({
        ...config,
        path,
...
>(
  {
    ...
    path:
      | FileManagementDownloadSharedOptions["path"]
      | (() => FileManagementDownloadSharedOptions["path"]);
  },
  options: {
    query?: Partial<
  ...
  const queryKey =
    resolvedOptions?.queryKey ?? fileManagementDownloadSharedQueryKey(resolvedParams);
