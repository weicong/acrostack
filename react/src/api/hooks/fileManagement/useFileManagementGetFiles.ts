...
import type {
  FileManagementGetFilesOptions,
  FileManagementGetFilesStatus200,
} from "../../models/fileManagement/FileManagementGetFiles";
import { queryOptions, useQuery } from "@tanstack/react-query";
import { fileManagementGetFiles } from "../../clients/fileManagement/fileManagementGetFiles";

export const fileManagementGetFilesQueryKey = ({
  query,
}: Omit<FileManagementGetFilesOptions, "headers"> = {}) =>
  [{ url: "/api/app/file-management/files" }, ...(query ? [query] : [])] as const;
...
export function fileManagementGetFilesQueryOptions(
  { query }: FileManagementGetFilesOptions = {},
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await fileManagementGetFiles({
        ...config,
        query,
...
>(
  {
    ...
    query?: FileManagementGetFilesOptions["query"] | (() => FileManagementGetFilesOptions["query"]);
  } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
