...
export const fileManagementGetThumbnailQueryKey = ({
  path,
}: Omit<FileManagementGetThumbnailOptions, "headers">) =>
  [{ url: "/api/app/file-management/files/:id/thumbnail", params: path }] as const;
...
export function fileManagementGetThumbnailQueryOptions(
  { path }: FileManagementGetThumbnailOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await fileManagementGetThumbnail({
        ...config,
        path,
...
>(
  {
    ...
    path:
      | FileManagementGetThumbnailOptions["path"]
      | (() => FileManagementGetThumbnailOptions["path"]);
  },
  options: {
    query?: Partial<
