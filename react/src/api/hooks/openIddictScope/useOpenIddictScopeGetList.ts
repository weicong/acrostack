...
export const openIddictScopeGetListQueryKey = ({
  query,
}: Omit<OpenIddictScopeGetListOptions, "headers"> = {}) =>
  [{ url: "/api/app/open-iddict-scope" }, ...(query ? [query] : [])] as const;
...
export function openIddictScopeGetListQueryOptions(
  { query }: OpenIddictScopeGetListOptions = {},
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await openIddictScopeGetList({
        ...config,
        query,
...
>(
  {
    ...
    query?: OpenIddictScopeGetListOptions["query"] | (() => OpenIddictScopeGetListOptions["query"]);
  } = {},
  options: {
    query?: Partial<
      QueryObserverOptions<
