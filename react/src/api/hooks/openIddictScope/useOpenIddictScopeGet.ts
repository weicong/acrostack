...
export const openIddictScopeGetQueryKey = ({ path }: Omit<OpenIddictScopeGetOptions, "headers">) =>
  [{ url: "/api/app/open-iddict-scope/:id", params: path }] as const;
...
export function openIddictScopeGetQueryOptions(
  { path }: OpenIddictScopeGetOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await openIddictScopeGet({
        ...config,
        path,
...
>(
  { path }: { path: OpenIddictScopeGetOptions["path"] | (() => OpenIddictScopeGetOptions["path"]) },
  options: {
    query?: Partial<
      QueryObserverOptions<
        OpenIddictScopeGetStatus200,
