...
export const conversationGetReactionsQueryKey = ({
  path,
}: Omit<ConversationGetReactionsOptions, "headers">) =>
  [{ url: "/api/app/conversation/reactions/:messageId", params: path }] as const;
...
export function conversationGetReactionsQueryOptions(
  { path }: ConversationGetReactionsOptions,
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await conversationGetReactions({
        ...config,
        path,
...
>(
  {
    ...
    path: ConversationGetReactionsOptions["path"] | (() => ConversationGetReactionsOptions["path"]);
  },
  options: {
    query?: Partial<
      QueryObserverOptions<
