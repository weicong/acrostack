...
export const conversationGetMessageListQueryKey = ({
  query,
}: Omit<ConversationGetMessageListOptions, "headers"> = {}) =>
  [{ url: "/api/app/conversation/message-list" }, ...(query ? [query] : [])] as const;
...
export function conversationGetMessageListQueryOptions(
  { query }: ConversationGetMessageListOptions = {},
  ...
  >({
    ...
    queryFn: async ({ signal }) => {
      const { data } = await conversationGetMessageList({
        ...config,
        query,
...
>(
  {
    ...
    query?:
      | ConversationGetMessageListOptions["query"]
      | (() => ConversationGetMessageListOptions["query"]);
  } = {},
  options: {
    query?: Partial<
